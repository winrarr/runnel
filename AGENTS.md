...
- For a performance-sensitive change, run `just bench-pr-local` after committing the change and paste its generated Markdown report into the pull request. Treat the same-host current-vs-default-branch median and observed workload as the primary local evidence; hosted PR benchmark jobs are informational sanity checks because shared runners are noisy.
...
- just bench-pr runs the short three-node clustered benchmark used as an informational pull-request sanity check.
- just bench-pr-local benchmarks the current commit and `origin/main` on the same host, aggregates repeated three-node results, and writes paste-ready Markdown under `benchmark-results/pr-local/`; use `just bench-pr-local -- --repetitions 1` for a quick smoke measurement.
