...
- Treat refactoring as part of normal engineering judgment for every agent, including delegated workers and coordinators. Whenever work touches a subsystem, inspect the touched code and its immediate surrounding structure for improvements that would make current or likely future work clearer, safer, or easier to change. If a refactor is safe and appropriately scoped, implement it in the current change. If it is something you would reasonably want to do later because it is broader, riskier, or requires coordination, record it as a focused tech-debt item in the same change, whether or not it crosses the current task boundary. Task narrowness alone is not a reason to omit that record. A no-update assessment is valid only when inspection found no concrete improvement worth recording, and the handoff must say so. Cross-boundary edits are appropriate when required for correctness or explicitly coordinated as one architectural effort. Tech-debt entries must state their goal, rationale, constraints, and verifiable retirement criteria. When work materially changes a backlog or tech-debt outcome, update that record in the same change. Every delegated handoff must include this refactor and planning-record assessment, and the coordinator must verify it before accepting the handoff.
