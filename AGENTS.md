...
- If the regular benchmark does not exercise the changed path, explicitly assess whether a targeted benchmark is feasible and proportionate. Run one when it is; when it would require disproportionate new harness work, decide whether correctness, resource, or operational value justifies merging without performance evidence. Defer performance claims, or revise/defer the change when benchmark evidence is necessary to justify it.
