...
- Treat refactoring as part of normal engineering judgment for every agent, including delegated workers and coordinators. While working in a subsystem, proactively identify structural improvements that would better support current or likely requirements. A substantial or higher-risk refactor may be implemented within the owned domain when the architectural benefit warrants it; do not reject sound evolution solely because it exceeds the immediate feature scope. If a worthwhile refactor crosses the current task boundary, add a focused tech-debt item for later rather than expanding into another task's area. Cross-boundary edits are appropriate when required for correctness or explicitly coordinated as one architectural effort. Tech-debt entries must state their goal, rationale, constraints, and verifiable retirement criteria. When work materially changes a backlog or tech-debt outcome, update that record in the same change; otherwise, state in the handoff why no refactor or planning-record update was warranted. Every delegated handoff must include this brief refactor and planning-record assessment. The coordinator must briefly remind the worker of this requirement at handoff and verify the assessment before accepting it.
