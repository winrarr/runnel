...
- For a performance-sensitive change, including delegated work, run `just bench-pr-local` after committing the change and paste its generated Markdown report into the pull request. The helper compares the current revision with `origin/main` on the same host inside an explicit Linux CPU/memory scope, repeats paired runs until the throughput and p99 stability thresholds are met or the maximum is reached, and records whether the result is stable or inconclusive. Treat only a stable report with its workload, limits, and revision metadata as performance evidence; `just bench-pr-local-quick` and hosted PR benchmarks are diagnostics, not proof.
