...
`.github/workflows/benchmarks.yml` runs three independent repetitions of all three suites for pushes to `main`, weekly scheduled runs, and manual runs: the native single-node comparison, the three-node RF=3 competitor publish comparison, and the three-node Runnel cluster baseline. It normalizes each repetition, aggregates each suite by median while retaining min/max observed values, preserves raw and aggregate results as workflow artifacts, and appends one record per suite plus generated `site/data.json` to the `benchmark-history` branch. The dashboard separates suites, displays repetition counts and ranges, and compares each run with the previous compatible workload, resource, broker-image, and measurement configuration. The native suite uses the `messages` input, the competitor cluster suite uses `cluster_comparison_messages`, and the Runnel cluster suite uses the smaller 200-message `cluster_messages` input by default because it exercises multiple durable Raft operations and recovery scenarios on a constrained runner. Set `messages`, `cluster_comparison_messages`, `cluster_messages`, and `repetitions` when dispatching the workflow manually.
