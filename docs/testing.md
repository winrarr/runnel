...
- `just bench-pr` runs the short three-node clustered benchmark used as an informational pull-request sanity check: 200 messages, 100 warmup messages, two parallel members, 100-byte payloads, and no restart-recovery delay. Use `just isolated bench-pr` when running it concurrently with another process-heavy workflow.
- `just bench-pr-local` is the canonical performance-sensitive PR workflow. It creates a detached worktree at `origin/main`, builds both revisions, alternates the same three-node workload on the same host, aggregates three repetitions per revision by median, and writes raw results, logs, and a paste-ready Markdown report under `benchmark-results/pr-local/`. Use `just bench-pr-local -- --repetitions 1` for a quick check, or pass workload flags after `--` when a change needs a different message size or volume. The current-vs-default-branch comparison is more useful for optimization decisions than an isolated hosted-runner point; record the generated report in the PR description.
...
- `python3 scripts/benchmarks/pr_report.py --input benchmark-results/pr-cluster.json --output benchmark-results/pr-cluster-comment.md --heading 'Clustered Runnel benchmark (primary)'` renders the clustered pull-request artifact as a compact Markdown report. The pull-request workflow also retains a single-node container report as a secondary diagnostic; both are informational rather than merge gates.
