...
- `just bench-pr` runs the short three-node clustered benchmark used as the primary pull-request performance signal: 200 messages, 100 warmup messages, two parallel members, 100-byte payloads, and no restart-recovery delay. Use `just isolated bench-pr` when running it concurrently with another process-heavy workflow.
