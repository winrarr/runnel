...
- Context: the inbound listener, framed connection dispatch, forwarded-operation execution, and forwarding error mapping now live in the private `crates/runnel-raft/src/network/inbound.rs` module. Focused transport tests and the real-process cluster smoke pass; outbound transport, connection-pool policy, and shared wire/framing ownership remain in `network.rs` for a later slice.
...
- Progress: HTTP router state, liveness/readiness handlers, bounded health checks, metric state, active-resource guards, delivery accounting, and Prometheus formatting now have private ownership in `crates/runnel-server/src/observability.rs`. Bootstrap, lifecycle, TCP framing/admission, request dispatch, and response mapping remain in `main.rs`, so this item stays open.
