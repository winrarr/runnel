...
- Progress: HTTP router state, liveness/readiness handlers, bounded health checks, metric state, active-resource guards, delivery accounting, and Prometheus formatting have private ownership in `crates/runnel-server/src/observability.rs`. TCP admission validation, bounded framing, request-line/deadline helpers, response serialization, connection rejection, and stable admission responses now have private ownership in `crates/runnel-server/src/protocol.rs`. Bootstrap/lifecycle, connection-task orchestration, request dispatch, and response mapping remain in `main.rs`, so this item stays open.
