...
This register records known implementation and documentation shortcomings. Product gaps belong in [backlog.md](backlog.md); an entry may link to the backlog item that will retire it. Entries are an inventory, not a priority order. Select work using current goals, dependencies, risk, and learning value. Keep remaining implementation work distinct from missing verification, and do not erase a useful partial improvement because the broader retirement gate remains open. Remove entries when the shortcoming is retired, and do not reuse their identifiers.

## TD-002: One file and a startup scan per local stream

- Status: open
- Impact: startup scans each complete stream file, older replay and candidate selection may require a linear scan, and retention cannot reclaim immutable regions independently. Record-location caches are bounded, but the request-identity map grows with distinct retained identities; bounded location indexing is not a bound on all retained metadata.
- Context: the one-file representation is intentionally the smallest durable log that can be tested end to end. The local engine keeps the newest 1,024 record locations and up to 1,024 sparse checkpoints, using the nearest available checkpoint for cold reads. Reads older than the sparse window can still scan from byte zero. Diagnostic replay benchmarks cross the tail-cache bound at 65,537 and 131,072 retained records; they do not provide segmented storage, retention, or storage-amplification evidence.
...
- Status: open
- Impact: current metrics expose process uptime, request rates and latency buckets, connections, traffic bytes, publish/delivery/acknowledgement totals, in-flight deliveries, redelivery, dead letters, storage bytes, configured admission limits, admission rejection counters, health failures, and snapshot and peer-transport activity. They still cannot explain consumer lag, retained and reclaimable storage, queue saturation, replication progress, or resource pressure under load.
- Context: the metrics endpoint now covers the basic broker and transport path, reports the shared engine's currently tracked in-flight deliveries for both local and clustered delivery, exposes the configured admission boundaries and their rejection counters, and keeps a label-free process-uptime signal available when bounded engine health is unavailable. Real-server tests cover in-flight saturation, slow readers, slow-writer recovery, uptime progression, and scrapeability during a health stall. An opt-in Rust timing feature and clustered profiling workflow provide deeper investigation data without adding timing calls to the default build, but those timings are not deployment-grade latency metrics.
...
- Status: open
- Impact: `runnel-raft` supports versioned local persistence, group-addressed framed TCP peer RPCs, a metadata group, one static data group per stream, reconciled stream activation, topology-free forwarding, stable stream metadata, durable publish request deduplication, preserved-state follower restart, leader failure, snapshot transfer with retry from byte zero, and snapshot lifecycle telemetry. Empty-replica recovery is test-only under [ADR 0018](decisions/0018-safe-replica-recovery-boundary.md). It still lacks a supported empty-replica replacement lifecycle, dynamic membership, scalable placement and balancing, efficient large-stream representation, final fencing policy, repeated-interruption cost controls, authentication, and production-grade operational policy. Replicating every stream to the same static voters is a useful three-node baseline but is not a scalable placement model.
...
- Status: open
- Impact: grouped polling searches candidate records and allows only one outstanding delivery per member. Local selection uses the bounded record cache or scans retained storage from a sparse checkpoint; clustered selection traverses materialized records. This keeps the first scheduler inspectable, but candidate work and the per-member delivery limit constrain throughput, batching, and scalability for large or highly concurrent worker pools.
...
- Context: local dead-letter movement still uses separate append-only stream logs and consumer checkpoints, but the target append uses the durable request-aware frame/index with an internal move identity and strict key/payload matching. The source acknowledgement remains after the durable target append or same-content reconciliation. The core test `dead_letter_move_reconciles_after_source_ack_persistence_failure_and_restart` injects a failure before source acknowledgement persistence, verifies unchanged source progress and one target record, then reopens the broker and verifies reconciliation. This is not injection during the underlying write or sync, nor a killed-process test. The clustered path already keeps the derived record and source progress in one replicated data-group transition.
...
- Context: persisting the attempt before returning a message makes retry behavior survive restart and keeps the first correctness model straightforward. The bounded append-only journal, replay, partial-tail recovery, compaction, active-state cache, stale-token fencing, and out-of-order acknowledgement coverage preserve those guarantees and produced an observed roughly 8% improvement in the focused benchmark; the durable write per delivery remains. A follow-up proposal to replace `sync_all` with `sync_data` for existing journal appends passed its replay checks but showed no repeatable improvement in controlled local comparisons, so closed PR #222 was not merged and does not change the debt status.
- Retirement condition: benchmarked batching, group commit, or another bounded bookkeeping strategy reduces delivery overhead without losing durable retry state, acknowledgement safety, or predictable recovery. A narrower sync primitive alone is insufficient without a material improvement on relevant workloads and filesystems.
...
- Context: replicated absolute deadlines establish a simple failover baseline without adding a separate lease service. Deterministic tests cover forward jumps, fixed leader offsets, deadline boundaries, restart and snapshot recovery, leader changes, no-command expiry, stale-token fencing, and the regression floor for a successor clock moving backward. The state machine persists a per-group maximum command-time observation, so expiry evaluation cannot move below that already observed time after recovery or a leader change. The floor does not advance with elapsed real time: a deadline beyond it may still be delayed by a regressed clock, and no command means no lazy expiry evaluation. Inter-node offsets and forward jumps also remain unresolved; this is not a bound on real-time lease error. Retry configuration and dead-letter provenance are tracked independently in TD-018, while retained delivery-state growth is covered by TD-010.
...
- Context: the first async boundary bound storage to 32 active and 32 queued operations with explicit `WouldBlock` admission, then centralized the boundary and added per-stream ordering and isolation. Focused tests cover ordering, cancellation, bounded admission, unrelated-stream progress, and storage-stall behavior, including bounded readiness failure and a scrapeable metrics fallback that omits unavailable engine samples. On 2026-09-04, the real-process FIFO-stall tests `storage_stall_is_bounded_and_durable_traffic_continues` and `storage_stall_shutdown_is_bounded_and_restart_recovers` passed in five serial repetitions (10 test cases) with a 500 ms request timeout, `<1 s` stalled-request bounds, `<2 s` readiness/metrics bounds, `<2 s` shutdown bound after releasing the FIFO, and restart recovery; each case used a unique temporary data directory and ephemeral listener ports. The timed-out same-stream waiter recovery test, prompt persistent-connection shutdown test, and `just isolated smoke` also passed. The repeated stall pair took 5.67–5.76 s per test run after compilation. These are bounded pass measurements rather than per-request latency samples: p99/p99.9 tail data, fixed CPU/memory-scoped slow-I/O measurements, and shutdown evidence for an actually uninterruptible filesystem call remain incomplete, so this debt stays open.
...
## TD-024: Exploratory documents mix durable approaches with implementation prescriptions

- Status: open
- Goal: preserve concrete, source-backed architectural exploration while making future implementation adaptable to current code and evidence.
- Impact: substantial notes such as `docs/design/application-aware-retry-policy.md`, `retention-disk-pressure-plan.md`, `single-node-to-cluster-migration.md`, and `stable-work-placement.md` mix useful mechanisms and alternatives with proposed API shapes, implementation stages, and baseline-specific observations. Readers must repeatedly distinguish examples from requirements. The distributed architecture research also names a strongest end-state candidate without comparative implementation evidence.
- Constraints: long-horizon research remains valuable regardless of implementation priority. Preserve sources, alternatives, failure reasoning, unresolved questions, and concrete ways an approach could work; do not shorten to a line quota or replace mechanisms with vague goals. Treat API and module sketches as illustrations unless accepted by an ADR. Preserve accepted decisions, including ADR 0023's storage and placement boundaries; any proposed change to their authority needs an explicit decision review rather than an editorial demotion.
- Retirement criteria: review the substantial research/design notes against their declared scope and current ADRs; clearly distinguish observations tied to a historical baseline, sourced facts, hypotheses, illustrative mechanisms, and accepted constraints. Replace prescriptive staged edits with outcome and evidence gates where they are not a current implementation plan. Link one authoritative home for repeated contracts. Qualify candidate rankings by their assumptions and missing evidence. Every revised note remains useful for comparing viable approaches without requiring the reader to follow an obsolete implementation recipe.

## TD-025: Shared engine errors expose implementation-specific failure details

- Status: open
- Goal: make engine failures express messaging outcomes independently of storage representation and cluster topology.
- Impact: `runnel-engine::BrokerError` mixes domain failures with `io::Error`, `serde_json::Error`, poisoned locks, leader IDs, and free-form cluster errors. Server translation hides some details from applications, but the shared engine contract remains coupled to backend mechanisms and requires careful mapping to preserve retry and unknown-outcome semantics.
- Constraints: inspect current engine, forwarding, server, and client mappings before choosing error categories. Preserve confirmed rejection versus unknown outcome, stale-delivery behavior, and useful diagnostic causes. Keep topology and persistence details available internally without making them application obligations. Do not add a generic error framework or silently change provisional wire codes; explicitly document any necessary compatibility change.
- Retirement criteria: local and clustered engines expose consistent semantic failure categories; backend-specific causes remain available for diagnosis; focused contract and real-server tests verify equivalent outcome classification, including timeouts and ambiguous writes. Documentation and compatibility fixtures match the accepted boundary, with an ADR for consequential contract changes.

## TD-026: Client response reads have a time bound but no byte bound

- Status: open
- Goal: bound client response memory while preserving supported payloads and explicit operation outcomes.
- Impact: `runnel-client::Client::request` reads a response into a growing `String` with `read_line` under a timeout. A peer that sends a large line or continuous bytes without a newline can grow the buffer until completion or timeout. Server-side request limits do not bound this client-side allocation.
- Constraints: derive a documented response limit from supported protocol shapes, including base64 expansion and batch outcomes, and expose configuration only where it has a real use. Reject oversized responses before buffering them in full, discard the affected connection, and classify an operation whose request was sent as unknown unless a complete response proves otherwise. Preserve cancellation and reconnect behavior.
- Retirement criteria: socket tests exercise oversized terminated and unterminated responses, fragmented valid responses at the limit, timeout/cancellation, and connection invalidation; a real-server test verifies the supported large-payload path. Reads have a demonstrable byte bound independent of the response timeout, and public configuration/error changes have compatibility documentation and fixtures as applicable.
