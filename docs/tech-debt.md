...
- Rationale: `scripts/benchmarks/compare.py` combined Docker service lifecycle, Kafka/Redpanda/JetStream command construction and parsing, Runnel execution, equivalence metadata validation, result aggregation, and CLI dispatch. Backend-specific changes could therefore affect common comparison policy, while the harness was difficult to extend with a new adapter or failure mode. This is a structural debt distinct from TD-013's unresolved semantic equivalence.
- Progress: native-tool images, command construction, readiness-output validation, and Kafka/NATS output parsing live in `scripts/benchmarks/compare_adapters.py`. This slice gives shared Docker/resource lifecycle and bounded cleanup focused ownership in `compare_lifecycle.py`, semantic metadata/result shaping and non-ranking guardrails in `compare_results.py`, backend execution orchestration in `compare_backends.py`, and argument/result-envelope dispatch in `compare_cli.py`; `compare.py` remains the stable executable/import facade. The real Docker comparison smoke path remains to be exercised after extraction, so this item stays open pending that evidence.
