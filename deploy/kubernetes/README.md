...
On SIGTERM or SIGINT, Runnel marks the pod unready, stops accepting new broker connections, and drains existing broker and HTTP work for up to 25 seconds before exiting. The StatefulSet's 30-second termination grace period leaves room for that local drain even if the Kubernetes control plane is slow to remove the pod from the client Service.
