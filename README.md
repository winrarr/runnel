...
`just bench-pr` runs the short three-node clustered benchmark used by pull requests as the primary optimization signal. Use `just isolated bench-pr` when running it concurrently with other process-heavy workflows. The benchmark skips restart recovery to keep feedback fast; the longer clustered history benchmark includes recovery separately.
