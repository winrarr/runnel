...
`just bench-compare-cluster` runs the verified first three-node RF=3 publish comparison against Kafka, Redpanda, and NATS JetStream with explicit resource limits. `just bench-dashboard` generates local benchmark history data from JSON results under `benchmark-results/`. The GitHub Actions benchmark workflow repeats the native competitor comparison, the three-node competitor comparison, and the three-node Runnel benchmark, aggregates each suite by median, keeps raw and aggregated results as artifacts, and appends the suite histories to the generated `benchmark-history` branch. The native, three-node competitor, and clustered suites have separate message-count inputs; the clustered workflow defaults to a smaller workload because it exercises multiple durable Raft operations and recovery scenarios. The dashboard also records the observed repetition range and compares each run with the previous compatible workload, resource, broker-image, and measurement configuration. GitHub Pages serves the hand-authored dashboard in `docs/benchmarks/` from `main`; the dashboard reads the public history data directly from that branch.
