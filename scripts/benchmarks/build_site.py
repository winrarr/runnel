...
def render_html(data: dict[str, Any], data_url: str = "data.json") -> str:
...
function populate() {
  ...
  const operations = unique(data.points.filter(point => point.metric === 'throughput_messages_per_second').map(point => point.operation)); operationSelect.replaceChildren(...operations.map(value => new Option(value, value))); if (operations.includes('publish')) operationSelect.value = 'publish';
...
'''
    return template.replace("__DATA_URL__", json.dumps(data_url)).replace("__DEFINITIONS__", definitions)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-url", default="data.json", help="URL loaded by the generated dashboard")
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--index-only", action="store_true", help="write only index.html")
    mode.add_argument("--data-only", action="store_true", help="write only data.json")
...
def main() -> int:
    ...
    if not args.data_only:
        (args.output / "index.html").write_text(
            render_html(data, data_url=args.data_url), encoding="utf-8"
        )
    output_kind = "index" if args.index_only else "data" if args.data_only else "site"
    print(f"generated {len(runs)} runs and {len(data['points'])} measurements in {output_kind} at {args.output}")
    return 0
